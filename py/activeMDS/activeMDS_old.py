...
class Embedding:
    ...
    def read_triplets(self):    
        ...
        self.label2index = dict( zip( self.labels,range(self.item_count) ) )
    
        self.S = []
        for queryLabels in Sl:
            q = [ self.label2index[x] for x in queryLabels ]
            self.S.append(q)
    ...
    def write(self,prefix):
        ...
        with open(embedding_file,'wb') as f:
            ...
            for i,(lab,x) in enumerate(zip(self.labels,self.X)):
                writer.writerow([i,lab]+x.tolist())

        query_file = "{}_queries.csv".format(
            prefix,
            self.total_epochs,
            int(self.proportion * 100),
            self.d,
